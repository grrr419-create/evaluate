...
- 관리자는 현재 평가의 제출 인원, 평가 기준, 현장 종합평가와 문항별 통계를 확인합니다. 현장 종합평가는 평균 `예` 응답과 참여자별 등급 인원을 함께 표시합니다. 대상 명단, 미참여자, 부서별 완료 조건은 없습니다. 제출이 1건 이상이면 통계와 다운로드가 제공됩니다.
...
`문항별 통계`와 참여자별 `개별 응답 001(양호)`, `개별 응답 002(보통)` 형식의 시트로 구성됩니다. 통계 시트 상단에는 `양호(우수)`, `보통`, `미흡` 인원과 구성비 및 전체 참여인원이 표시되며, 아래에는 문항별 `예`, `아니오` 인원과 비율이 표시됩니다. 개별 응답 시트 제목에는 판정과 기준이 표시되고, 답변 표 아래에는 해당 응답에 따른 현재 상태와 평가 내용이 표시됩니다. `아니오` 답변은 연한 적색으로 강조합니다. 평가 이름·문항번호 열·2행 안내 문구는 포함하지 않습니다. 시트 번호는 임의 UUID 정렬로 정하며 제출 순서를 뜻하지 않습니다. 텍스트는 수식으로 실행되지 않는 문자열 셀로 저장합니다.

판정 기준은 14개 문항의 `예` 응답 수를 사용합니다. `양호(우수)`는 12개 이상, `보통`은 9개~11개, `미흡`은 0개~8개입니다. 관리자 화면과 엑셀 다운로드는 `public/assessment-rules.js`의 같은 기준과 평가 문구를 사용합니다.
