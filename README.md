...
- 관리자 화면: `admin.html`. 참여·미참여 명단을 확인하고, 부서 전원이 제출한 경우에만 해당 부서의 문항별 통계와 엑셀 다운로드를 제공합니다.
- 결과 초기화는 관리자 로그인과 확인창을 거쳐 실행합니다. 참여 기록과 집계 결과가 함께 초기화되고, 진행 중인 평가 세션도 만료됩니다.
...
| `supabase/migrations/202609030001_evaluate.sql` | 비공개 데이터 구조와 권한, 트랜잭션, 통계 공개 조건 |
...
평가 결과는 개인별 답변 묶음을 저장하지 않고, 부서·문항·선택지별 합계와 별도의 참여 표시로 저장합니다. 제출과 집계를 한 트랜잭션으로 처리하며, 부서 구성원 전원이 참여하기 전에는 API에서도 통계와 다운로드를 거부합니다. 명단은 관리자 권한으로만 조회할 수 있습니다.

## 설치·배포

1. Supabase에서 전용 `evaluate` 프로젝트를 만듭니다.
2. SQL Editor에서 마이그레이션 파일을 실행합니다.
...
테스트는 PGlite의 실제 Postgres 엔진과 pgcrypto를 사용하며, 운영 프로젝트에 평가를 제출하거나 결과를 초기화하지 않습니다. 검증 대상은 권한 분리, 동시 중복 제출, 모든 문항의 유효성, 부서 완료 조건, 초기화 후 세션 무효화, 로그인 제한, CORS, 요청 크기 제한, XLSX 생성입니다.
