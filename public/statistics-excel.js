...
const StatisticsExcel=(()=>{
 ...
 function sheet({title,headers,rows,widths,countColumns=[],landscape=false}){
  ...
  const xml=values.map((row,index)=>{
   const height=index===0?36:index===1?30:Math.max(36,...row.map((v,i)=>Math.ceil(textWidth(v)/Math.max(8,widths[i]-3))*17+10));
   return '<row r="'+(index+1)+'" ht="'+height+'" customHeight="1">'+row.map((value,column)=>{
    const ref=String.fromCharCode(65+column)+(index+1),style=index===0?1:index===1?2:countColumns.includes(column)?5:column===0?3:4;
  ...
  return declaration+'<worksheet xmlns="'+mainNamespace+'"><sheetPr><pageSetUpPr fitToPage="1"/></sheetPr><dimension ref="A1:'+last+values.length+'"/><sheetViews><sheetView workbookViewId="0" showGridLines="0"><pane ySplit="2" topLeftCell="A3" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols>'+widths.map((w,i)=>'<col min="'+(i+1)+'" max="'+(i+1)+'" width="'+w+'" customWidth="1"/>').join('')+'</cols><sheetData>'+xml+'</sheetData><autoFilter ref="A2:'+last+values.length+'"/><mergeCells count="1"><mergeCell ref="A1:'+last+'1"/></mergeCells><pageMargins left="0.3" right="0.3" top="0.5" bottom="0.5" header="0.2" footer="0.2"/><pageSetup paperSize="9" orientation="'+(landscape?'landscape':'portrait')+'" fitToWidth="1" fitToHeight="0"/></worksheet>';
 }
 function create(dep){
  ...
  const labels=['예','아니오'].filter(label=>available.includes(label)).concat(available.filter(label=>!['예','아니오'].includes(label)));
  const rows=dep.statistics.map(q=>[q.text,dep.completed,...labels.map(label=>{
   const index=q.options.indexOf(label);
   if(index<0)return '—';
   const count=q.counts[index],percent=Math.round(count/dep.completed*1000)/10;
   return count+'명 ('+percent+'%)';
  })]);
  const sheets=[{name:'문항별 통계',title:'문항별 통계',headers:['문항','참여',...labels],rows,widths:[90,14,...labels.map(()=>24)],countColumns:[1],landscape:true}];
  dep.responses.forEach((response,i)=>{
   const number=String(i+1).padStart(3,'0');
   sheets.push({name:'응답 '+number,title:'개별 응답 '+number,headers:['문항','선택 답변'],rows:dep.statistics.map(q=>[q.text,q.options[response.answers[q.id]]]),widths:[90,24]});
  });
  const files=[
