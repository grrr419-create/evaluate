...
test('deployed API uses explicit DELETE/UPDATE predicates required by PostgREST safeupdate', async () => {
  ...
  try {
    const sql = (
      await db.query(
        "select prosrc from pg_proc where oid='public.evaluate_api(text,jsonb,text,text)'::regprocedure",
      )
    ).rows[0].prosrc;
...
test('reset removes upgraded aggregate-only data and ignores retired assessment-name input', async () => {
  ...
  try {
    ...
    await db.exec(
      "update evaluate_private.settings set legacy_count=1,assessment_name='obsolete name' where singleton; insert into evaluate_private.legacy_counts values('question-a',0,1),('question-b',1,1);",
    );
    ...
    const reset = await call(
      db,
      '/api/admin/reset',
      { token, confirmation: '정말로 초기화 하시겠습니까?', name: 'ignored legacy client field' },
      admin,
    );
    assert.equal(reset.status, 200);
    const after = (await call(db, '/api/admin/dashboard', {}, admin)).data;
    assert.equal(after.completed, 0);
    assert.equal(after.name, '업무환경 심리평가');
    ...
    assert.equal((await call(db, '/api/admin/dashboard', {}, admin)).data.epoch, after.epoch);
  } finally {
    await db.close();
  }
});
