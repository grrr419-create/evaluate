...
test('public roles cannot read answers; limits expire and allow shared-IP colleagues',async()=>{
 const db=await database();try{
  for(const role of ['anon','authenticated']){
   ...
   for(const table of ['settings','round_responses','admissions'])await assert.rejects(db.query('select * from evaluate_private.'+table),/permission denied/);
...
test('Edge gateway checks origins, size, routes and private role authentication',async()=>{
 const db=await database();try{
  ...
  const handle=createHandler({ALLOWED_ORIGINS:'https://example.github.io',SUPABASE_URL:'https://project.supabase.co',SUPABASE_SERVICE_ROLE_KEY:'fixture-only'},async(url,opts)=>{
   calls++;assert.equal(url,'https://project.supabase.co/rest/v1/rpc/evaluate_api');
   const input=JSON.parse(opts.body);return Response.json(await call(db,input.p_route,input.p_body,input.p_session,input.p_client));
  });
